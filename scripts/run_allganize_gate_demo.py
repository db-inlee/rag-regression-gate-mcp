"""D3 — engine-reuse proof: run the SAME regression gate on the Allganize domain.

1. measure an allganize noise band (N runs, baseline config) -> examples/allganize_baseline/
2. build a weakened candidate (top_k 5->1, search degraded)   -> examples/allganize_candidate/
3. judge baseline vs candidate by calling the ENGINE directly:
   ...
   (importing & running the UNCHANGED engine IS the reuse proof — 3rd domain.)

The engine is gold-free / domain-free: it consumes only the enriched attribution
(gate_fields) + noise_band — identical format to DART/Wiki. No engine edits.

Note: Allganize gold matching is DOCUMENT-level (coarser than DART), so a top_k cut
may move retrieval_miss less than DART; the answerable/hallucination signal can be
the dominant one. We report whatever the engine finds, honestly.
"""
...
from app.adapters.allganize import (ALLGANIZE_CONFIG, allganize_band_vector,
                                    allganize_metrics, build_allganize_index,
                                    evaluate_allganize)
...
CAND = Path("examples/allganize_candidate")
N_RUNS = 3
...
def main() -> int:
    ...
    require_openai_key()
    client = OpenAI()
    build_allganize_index(ALLGANIZE_CONFIG)

    # 1) baseline noise band: N runs at baseline config (temp=0+seed → honest small band)
    print(f"[1] measuring allganize noise band: {N_RUNS} runs @ top_k={ALLGANIZE_CONFIG.top_k} ...")
    vectors, run_ids, first = [], [], None
    for r in range(1, N_RUNS + 1):
        cases, records, attrs = evaluate_allganize(ALLGANIZE_CONFIG, client=client)
        vectors.append(allganize_band_vector(attrs))
        run_ids.append(f"allganize_noise_{ALLGANIZE_CONFIG.index_signature()}_r{r}")
        if first is None:
    ...
    base_m = _write_run(BASE, run_ids[0], ALLGANIZE_CONFIG, cases, records, attrs)
    (BASE / "noise_band.json").write_text(json.dumps(
        {"n_runs": N_RUNS, "domain": "allganize", "config": ALLGANIZE_CONFIG.fingerprint(),
         "run_ids": run_ids, "metric_band": metric_band}, ensure_ascii=False, indent=2), encoding="utf-8")
    (BASE / "meta.json").write_text(json.dumps({"run_id": run_ids[0], "domain": "allganize",
        "config": ALLGANIZE_CONFIG.fingerprint(), "n_cases": len(cases)}, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    # 2) candidate: weaken retrieval (top_k 5 -> 1). Same index (top_k is query-time).
    cand_cfg = ALLGANIZE_CONFIG.model_copy(update={"top_k": 1})
    print(f"[2] candidate run @ top_k={cand_cfg.top_k} (search weakened) ...")
    c_cases, c_records, c_attrs = evaluate_allganize(cand_cfg, client=client)
    cand_m = _write_run(CAND, f"allganize_cand_topk1_{cand_cfg.index_signature()}", cand_cfg,
    ...
    print("\n[3] running the UNCHANGED engine on the allganize domain ...")
    report = detect_paths(BASE, CAND)
    gate = evaluate(report)
    print(render(report, gate))
    out = Path("gate_runs") / "gate_allganize_candidate.json"
    ...
    print(f"answerable_acc  baseline={base_m['answerable_accuracy']}  →  candidate={cand_m['answerable_accuracy']}")
